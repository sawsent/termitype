...
from termitype.models.engine.run import Run, RunSegment
...
class TypingEngine:
    ...
    def new_run(self, word_count: int):
        words = [ random.choice(self.word_list) for _ in range(word_count) ]
    ...
    def type_char(self, char: str) -> None:
        ...
        if self.current_run is not None and self.current_run.in_play:
            ...
            if len(self.current_run.word_list) == len(self.current_run.words_typed):
    ...
    def current_run_state(self) -> List[List[RunSegment]]:
        if self.current_run is not None:
            return self.current_run.get_segments()
        else:
            return []
